...
def Project_A(user_name):
    ...
    while True:
        time.sleep(1)
        text_to_speech(
            f"안녕 {user_name}{aa(user_name)}. 나는 인간에게 다양한 서비스를 제공하기 위해서 만들어진 서비스 로봇이야. 지금부터 내가 무엇을 할 수 있는지 설명해줄게.")
        text_to_speech(
            "안녕! 내 이름은 파이보야. 나는 반려로봇으로 개발 되었고 휴먼 로봇 인터렉션 연구에 적용되어 사용되고 있어")
        text_to_speech(
    ...
    while True:
        ...
        if user_said == '멋있는':
            while True:
                ...
                # 유저가 무엇을 선택했는지 기록
                text_to_speech(
                    f"{user_name}{lee(user_name)}이는 멋있는 스티커를 좋아하는구나. 좋은 선택이야!")
                break
        else:
            while True:
                time.sleep(1)
                text_to_speech(
                    f"{user_name}{lee(user_name)}이는 귀여운 스티커를 좋아하는구나. 좋은 선택이야!")
                break
        break
